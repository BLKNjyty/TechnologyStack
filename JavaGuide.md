...
> 只有大括号包围住代码。这种情况从形式上看，还分为两种：
>
> 一种出现在定义类的域的位置，这个里面的代码每次生成一个对象时，都会执行一次，而且比构造函数优先执行，例子如下：
>
> {
...
> }
>
> 另一种出现在类的方法代码块中，起到了限定作用域的功能，定义在该代码块中的变量，不能被所在同一方法下该代码块以外访问，可以进一步将这段代码块提取出来成为独立的方法
...
- 初始标记：STW，也是只标记GC Roots直接关联的对象，并修改TAMS的指针值（G1为每一个Region设计了两个名为TAMS（Top at Mark Start）的指针，把Region中的一部分空间划分出来用于并发回收过程中的新对象分配，并发回收时新分配的对象地址都必须要在这两个指针位置以上，垃圾回收时也不会回收这部分空间），这个过程耗时很短，而且是借用进行 Minor GC 的时候同步完成的，所以 G1 收集器在这个阶段实际并**没有额外的停顿**。
...
![img](images\笑脸.png)**！！！！G1停顿时间的瓶颈主要是标记-复制中的转移阶段STW。(为什么转移阶段不能和标记阶段一样并发执行呢？主要是G1未能解决转移过程中准确定位对象地址的问题(即并发标记的时候对象的引用可能会发生变化，所以没法转移和标记并发执行)。)**
...
于是**在 Redis 重启的时候，可以全量先加载 rdb 的内容，然后再重放增量 AOF 日志就可以完全替代之前的 AOF 全量文件重放，重启效率因此大幅得到提升**。
...
1、这样一旦MySQL中产生了新的写入、更新、删除等操作，就可以把binlog相关的消息推送至MQ

2、MQ再将消息发送给Redis

3、Redis再根据binlog中的记录，对Redis进行更新

4、其实这种机制，很类似MySQL的主从备份机制，因为MySQL的主备也是通过binlog来实现的数据一致性
